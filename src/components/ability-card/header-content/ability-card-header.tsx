...
export function AbilityCardHeader({card}: { card: ability_card }) {
    const cardColourSettings = useAppSelector(selectCardTypeSettingsByCardType(card.type)) ?? {}
                ...
                <div className={`${card.hasCost ? 'w-[190pt]' : 'w-[219pt]'} h-[10pt]`}>
                  {!card.topMatterFontSizeOverride ?
                    ...
                    <h2 className={`text-[10pt] font-display font-bold small-caps leading-none indent-[1.8pt]`}
                        style={{color:getDynamicColorBase(card.type, cardColourSettings, baseColours)}}><AutoTextSize maxFontSizePx={14}>{card.topMatter}</AutoTextSize></h2>
                    :
                    // New Flow: Uses top matter fontsize override
                    <h2 className={`font-display font-bold small-caps leading-none indent-[1.8pt]`}
                        style={{color:getDynamicColorBase(card.type, cardColourSettings, baseColours), fontSize: card.topMatterFontSizeOverride}}>{card.topMatter}</h2>
                  }
                </div>
                <div className={`${card.hasCost ? 'w-[190pt]' : 'w-[219pt]'} h-[14pt]`}>
                  {!card.titleFontSizeOverride ?
                    ...
                    <h1 className={`${card.hasCost ? 'w-[190pt]' : 'w-[219pt]'} text-[18pt] font-display font-bold small-caps leading-[13pt]`}
                        style={{color:getDynamicColorBase(card.type, cardColourSettings, baseColours)}}><AutoTextSize maxFontSizePx={22}>{card.title}</AutoTextSize></h1>
                    :
                    ...
                    <h1 className={`${card.hasCost ? 'w-[190pt]' : 'w-[219pt]'} font-display font-bold small-caps leading-[13pt]`}
                        style={{color:getDynamicColorBase(card.type, cardColourSettings, baseColours), fontSize: card.titleFontSizeOverride}}>{card.title}</h1>
                  }
                </div>
                  ...
                  <div className={`h-[20pt]`}>
                    {!card.flavourFontSizeOverride ?
                      ...
                      <div className={`text-[9pt] h-full font-body italic font-light leading-none`}
                           style={{color:getDynamicColorBase(card.type, cardColourSettings, baseColours)}}><AutoTextSize mode="box" maxFontSizePx={17}>{card.flavour}</AutoTextSize></div>
                      :
                      // New Flow: Uses flavour fontsize override
                      <div className={`h-full font-body italic font-light leading-none`}
                           style={{color:getDynamicColorBase(card.type, cardColourSettings, baseColours), fontSize: card.flavourFontSizeOverride}}>{card.flavour}</div>
                    }
                  </div>
                }
            </div>
