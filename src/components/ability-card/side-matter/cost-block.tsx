...
export function CostBlock({card}: {card: ability_card}) {
    if (!card.hasCost) {
        return (<div className={`w-[54pt] h-[16pt]`}></div>);
    }
    return (
        <div className={`w-[54pt] h-[16pt] ${actionBg100ColorStyle[card.type]} flex flex-col justify-center rounded-tr-[5pt] rounded-bl-[4pt]`}>
