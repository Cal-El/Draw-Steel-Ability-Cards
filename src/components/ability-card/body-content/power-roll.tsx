...
function powerRollLine(card: ability_card, powerRollTier: power_roll_tier, rowNum: number) {
    return <div className={`flex w-full`}>
        <div className={`${actionBg100ColorStyle[card.type]} w-[8.1pt] h-[33pt] flex justify-center`}>
        ...
        {powerRollTier.hasDamage ?
            <div className={`relative  ${rowNum % 2 === 1 ? actionBg50ColorStyle[card.type] : actionBg40ColorStyle[card.type]} ${powerRollTier.damageValue !== undefined && powerRollTier.damageValue?.length > 2 ? `w-[42pt]` : `w-[33pt]`} h-[33pt]`}>
        ...
        {powerRollTier.hasGeneralEffect ?
            <div className={`${rowNum % 2 === 1 ? actionBg20ColorStyle[card.type] : actionBg30ColorStyle[card.type]} flex-1 flex h-[33pt]`}>
                <div className={`h-[33pt] flex-1 flex flex-col justify-center pl-[3.5pt] pr-[1pt]`}>
                    <div className={`${powerRollTier.generalEffect && powerRollTier.generalEffect?.length < 81 || !powerRollTier.hasPotency && powerRollTier.generalEffect && powerRollTier.generalEffect?.length < 161? `text-[9pt]` : `text-[7.5pt]`} font-body ${actionTextColorStyle[card.type]} leading-none text-left`}>{powerRollTier.generalEffect}</div>
                </div>
            </div> : <></>
        }
        {powerRollTier.hasPotency ?
            <div className={`relative  ${rowNum % 2 === 1 ? actionBg40ColorStyle[card.type] : actionBg50ColorStyle[card.type]} w-[42pt] h-[33pt]`}>
        ...
        {powerRollTier.hasPotency ?
            <div className={`${rowNum % 2 === 1 ? actionBg20ColorStyle[card.type] : actionBg30ColorStyle[card.type]} flex-1 flex h-[33pt]`}>
                <div className={`h-[33pt] flex-1 flex flex-col justify-center pl-[3.5pt] pr-[1pt]`}>
                    <div className={`${powerRollTier.potencyEffect && powerRollTier.potencyEffect?.length < 81 || !powerRollTier.hasGeneralEffect && powerRollTier.potencyEffect && powerRollTier.potencyEffect?.length < 161? `text-[9pt]` : `text-[7.5pt]`} font-body ${actionTextColorStyle[card.type]} leading-none text-left`}>{powerRollTier.potencyEffect}</div>
                </div>
            </div> : <></>
        }
    </div>
...
export function PowerRollStatement({card, powerRoll}: {card: ability_card, powerRoll: power_roll_statement}) {
    return (
        <div className={`flex flex-col h-[114pt] justify-center gap-y-[3pt]`}>
            <div className={`flex h-[12pt]`}>
            ...
            <div className={`flex flex-col w-full h-[99pt] ${actionBg100ColorStyle[card.type]}`}>
