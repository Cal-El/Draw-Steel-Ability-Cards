  ...
  const cardTypeSettings = useSelector(selectCardTypeSettingsByCardType(card.type)) ?? {}
    ...
    return (
        <div className={`flex-auto flex ${useBacking && `py-[3pt]`} w-full`}
             style={useBacking ? {backgroundColor: getDynamicColor20(card.type, cardTypeSettings)} : {}}>
          ...
          {!card.bodyFontSizeOverride ?
            ...
            <p key={'autosized'} className={`max-w-[217pt] text-[9pt] font-body leading-none whitespace-pre-line`}
               style={{color:getDynamicColorBase(card.type, cardTypeSettings)}}><AutoTextSize minFontSizePx={4} maxFontSizePx={18} mode="box"><Markdown>{(key?.length > 0 ? "**" + key.replace('#', '') + "**: " : "") + kv.value}</Markdown></AutoTextSize></p>
            :
            // New Flow: Uses body fontsize override
            <p key={'override'} className={`max-w-[217pt] font-body leading-none whitespace-pre-line`}
               style={{color:getDynamicColorBase(card.type, cardTypeSettings), fontSize: card.bodyFontSizeOverride}}><Markdown>{(key?.length > 0 ? "**" + key.replace('#', '') + "**: " : "") + kv.value}</Markdown></p>
          }
        </div>
    )
}
