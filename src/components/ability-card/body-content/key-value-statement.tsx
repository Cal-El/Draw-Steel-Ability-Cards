  ...
  const colourSettings = useAppSelector(selectColourSettings)
    ...
    return (
        <div className={`flex-auto flex ${useBacking && `py-[3pt]`} w-full`}
             style={useBacking ? {backgroundColor: getDynamicColor20(card.type, colourSettings)} : {}}>
          ...
          {!card.bodyFontSizeOverride ?
            ...
            <p key={'autosized'} className={`max-w-[217pt] text-[9pt] font-body leading-none whitespace-pre-line`}
               style={{color:getDynamicColorBase(card.type, colourSettings)}}><AutoTextSize minFontSizePx={4} maxFontSizePx={18} mode="box"><Markdown>{(key?.length > 0 ? "**" + key.replace('#', '') + "**: " : "") + kv.value}</Markdown></AutoTextSize></p>
            :
            // New Flow: Uses body fontsize override
            <p key={'override'} className={`max-w-[217pt] font-body leading-none whitespace-pre-line`}
               style={{color:getDynamicColorBase(card.type, colourSettings), fontSize: card.bodyFontSizeOverride}}><Markdown>{(key?.length > 0 ? "**" + key.replace('#', '') + "**: " : "") + kv.value}</Markdown></p>
          }
        </div>
    )
}
