...
import {
  getKeywordColor, getKeywordTextColor, getPrimaryColor, getTextColourOnBackground, getTextColourOnFadedPrimary
  ...
  return <div id={'header'} className={`w-full h-min flex pl-[1pt] pr-[7pt]`}>
    <div id={`cost`} className={`w-[12pt] h-0 flex-none overflow-visible`}>
      <div className={`w-[12pt] h-[28pt] flex justify-center items-center`}>
        <div style={{color: getTextColourOnFadedPrimary(card.type, colourSettings), letterSpacing: '-2pt', textIndent: '-2pt', fontSize: '10pt', fontWeight: 'bold', textAlign: 'center', lineHeight: '9pt', fontVariantNumeric: 'lining-nums'}}>{card.cost?.costValue}</div>
      </div>
    </div>
    <div id={`header-title-and-flavour`}>
      <div style={{color: getTextColourOnBackground(card.type, colourSettings), fontVariantCaps: 'small-caps', fontSize: card.v2FontSizePtOverrides?.titleFont || '13pt', fontWeight: 'bold', lineHeight: '10.5pt'}}>{card.header.title}</div>
      {card.header.flavour ? <>
        <div style={{color: getTextColourOnBackground(card.type, colourSettings), paddingTop: '0pt', paddingBottom: '1pt', fontSize: card.v2FontSizePtOverrides?.flavour || '8pt', fontWeight: 'normal', fontStyle: 'italic', lineHeight: '7.5pt', lineClamp: 2, textIndent: '3pt'}}>{card.header.flavour}</div>
